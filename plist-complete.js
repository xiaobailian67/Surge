...
function parseXml(xmlString) {
  ...
  return children.length === 1 ? parseXmlNode(children[0]) : children.map(parseXmlNode);
}
...
function parseBinary(buffer) {
  ...
  const trailer = new DataView(data.buffer, data.byteOffset + trailerOffset, 32);
  ...
  const parser = new BinaryPlistParser(data, offsetTable, objectRefSize, parsedObjects);
...
class BinaryPlistParser {
  ...
  parseObject(objIndex) {
    ...
    switch (type) {
      case 0x0:
        ...
        else throw new Error(`Unknown primitive type: 0x${marker.toString(16)}`);
        break;
      case 0x1:
  ...
  parseInteger(offset, byteCount) {
    if (byteCount === 1) {
      ...
      return new DataView(this.data.buffer, this.data.byteOffset + offset, 2).getUint16(0);
    } else if (byteCount === 4) {
      return new DataView(this.data.buffer, this.data.byteOffset + offset, 4).getUint32(0);
    } else if (byteCount === 8) {
  ...
  parseReal(offset, byteCount) {
    const view = new DataView(this.data.buffer, this.data.byteOffset + offset, byteCount);
    if (byteCount === 4) {
  ...
  parseUtf16String(offset, info) {
    ...
    let str = '';
    for (let i = 0; i < length; i++) {
      const charCode = new DataView(this.data.buffer, this.data.byteOffset + strOffset + i * 2, 2).getUint16(0);
  ...
  parseArray(offset, info) {
    ...
    for (let i = 0; i < count; i++) {
      const objRef = readInteger(this.data, objOffset + i * this.objectRefSize, this.objectRefSize);
  ...
  parseDict(offset, info) {
    ...
    for (let i = 0; i < count; i++) {
      const keyRef = readInteger(this.data, keyOffset + i * this.objectRefSize, this.objectRefSize);
      const valRef = readInteger(this.data, valOffset + i * this.objectRefSize, this.objectRefSize);
...
class BinaryPlistBuilder {
  ...
  build(rootObj) {
    ...
    const totalSize = header.length + objectData.reduce((sum, d) => sum + d.length, 0) + offsetTable.length + trailer.length;
