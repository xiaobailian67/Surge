...
class HttpClient {
  ...
  static _adapters = new Set([
    // 兼容Quantumult X
    ({ $auto, ...op }, handleRes) =>
      $task.fetch({ opts: { redirection: $auto }, ...op }).then(handleRes, handleRes),
  ...
  get coreHooks() {
    const set = (obj) => {
      ...
      Object.keys(obj).forEach((key) => {
        if (!obj[key].isDefault && this._coreHooks[key]?.isDefault) {
          obj[key].remove();
          throw new Error(`核心钩子 '${key}' 已存在且尝试以非默认方式覆盖，操作被阻止。`);
        }
      });
    ...
    const get = (input) => {
      ...
      return Object.fromEntries(
        Object.entries(this._coreHooks).map(([key, value]) => [key, value.status()])
      );
    };
